...
namespace Catalog.Messages
{
    public partial class ConversationsPage
    {
        ...
        private async Task<TableData<ConversationDto>> ServerReload(TableState state)
        {
            try
            {
                var results = await ConversationsClient.GetConversationsAsync(state.Page, state.PageSize, state.SortLabel, state.SortDirection == MudBlazor.SortDirection.Ascending ? Messenger.Client.SortDirection.Asc : Messenger.Client.SortDirection.Desc);
